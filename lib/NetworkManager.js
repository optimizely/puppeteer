...
class NetworkManager extends EventEmitter {
  ...
  /**
   * @param {boolean} value
   * @param {Array} urlPatterns List of RequestPatterms
   * https://chromedevtools.github.io/devtools-protocol/tot/Network#type-RequestPattern
   */
  async setRequestInterception(value, reqPatterns) {
    this._userRequestInterceptionEnabled = value;
    await this._updateProtocolRequestInterception(reqPatterns);
  }

  async _updateProtocolRequestInterception(reqPatterns) {
    const enabled = this._userRequestInterceptionEnabled || !!this._credentials;
    if (enabled === this._protocolRequestInterceptionEnabled)
      return;
    this._protocolRequestInterceptionEnabled = enabled;
    const patterns = enabled ? reqPatterns || [{urlPattern: '*'}] : [] ;
    await Promise.all([
